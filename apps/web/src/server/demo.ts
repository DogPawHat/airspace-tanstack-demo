...
const env = (key: string): string | undefined =>
  (import.meta.env as Env)[key] ?? process.env[key]
...
async function inviteFor(describe: { inviteCodeRequired?: boolean }): Promise<string | undefined> {
  if (!describe.inviteCodeRequired)
    return undefined
  if (env('AIRSPACE_INVITE_CODE'))
    return env('AIRSPACE_INVITE_CODE')
  ...
  inviteCode ??= await (async () => {
    ...
    if (!created.ok)
      throw new Error(`could not mint an invite code: ${await created.text()}`)
...
export async function createAccount(): Promise<DemoAccount> {
  ...
  try {
    describe = await (await fetch(`${service()}/xrpc/com.atproto.server.describeServer`)).json() as typeof describe
  }
  catch {
...
async function build(account: DemoAccount) {
  const session = await passwordSession({ service: service(), identifier: account.handle, password: account.password })
...
export const useDemoAirspace = async (): Promise<DemoAirspace> => (await instanceFor(await requireAccount())).airspace
