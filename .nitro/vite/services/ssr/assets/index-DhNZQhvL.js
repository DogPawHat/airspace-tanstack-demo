...
const CODE_POINT_MASK = 2097151;
const CONSUMED_OVERFLOW = 2047;
...
function* walkGenerator(document, checker) {
  function* walk(node, parent, index) {
    ...
    if (Array.isArray(currentNode) && currentNode.length > 2) {
      let i2 = 2;
      while (i2 < currentNode.length) {
        const childRemoved = yield* walk(currentNode[i2], currentNode, i2);
        if (childRemoved) {
          continue;
        }
        i2 += 1;
...
function visit(document, checker, visitor) {
...
function splitByCase(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    const isNumber2 = charCode >= 48 && charCode <= 57;
    const isUpper = isNumber2 ? void 0 : charCode >= 65 && charCode <= 90;
